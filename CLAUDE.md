...
- The **custom keyboard** has 3 pages (QWERTY, 123, symbols) plus a macro page via long-press on the fn button. Keys show an iOS-style callout on press. Keys with alternates (e.g. long-press `e` for accented variants) show a picker strip. Double-tap space inserts a period (iOS-style), long-press space sends Tab.
